...
describe 'Bitbucket payload' do
  ...
  it 'should output 500 no command received' do
    authorize user, pass
    ...
    assert last_response.body.must_match "no command configured"
  end
  # payload has a known repository and unknown branch and no '_all' configuration
  it 'should output 500 no configuration for branch unknownbranch found' do
    authorize user, pass
    ...
    assert_equal 500, last_response.status
    assert last_response.body.must_match "no configuration for branch 'unknownbranch' in repository 'encdata' found"
  end
  # payload has a unknown repository
  it 'should output 500 the repository unknownrepo is not configured' do
    authorize user, pass
    ...
    assert_equal 500, last_response.status
    assert last_response.body.must_match "the repository 'unknownrepo' is not configured"
  end
end
